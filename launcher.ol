...
from file import File,FileNotFoundType
...
/* 
The Slicer service is loaded dynamically to avoid issues symlinks in the 
interpreter. Because of this, jolie-slicer.ol is not shipped with the pkg.
(cf. .npmignore)
...
options:
  ...
  main {
    ...
    scope( usage ) {
      ...
      if( !endsWith@str( HOME{ suffix -> FILE_SEPARATOR } ) ) {
        HOME += FILE_SEPARATOR
      }
      if (!is_defined( params.templatesDir ) ) {
        params.templatesDir = HOME + DEFAULT_TEMPLATES_DIR
      }
    ...
    if(params.mode == MODE_SIMULATE) {
      ...
      loadLibrary@runtime( HOME + "lib" + FILE_SEPARATOR + "jolieslicer.jar" )()
      loadEmbeddedService@runtime( {
        filepath = "joliex.slicer.JolieSlicer"
        type = "java"
      } )(slicer.location)
